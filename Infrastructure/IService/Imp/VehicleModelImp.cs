...
namespace Infrastructure.IService.Imp
{
    public class VehicleModelImp : IVehicleModelService
    {
        ...
        public async Task<ResponseVehicleModel> CreateNewVehicleModel(CreateVehicleModel vehicleModel)
        {
            ...
            return _mapper.Map<ResponseVehicleModel>(model);
        }



        public async Task<List<ResponseVehicleModel>> GetAllVehiclesModels()
        {
            var models = await _unitofWork.VehicleModel.GetAll();
            return _mapper.Map<List<ResponseVehicleModel>>(models);
        }

        public async Task<ResponseVehicleModel> GetVehicleById(Guid id)
        {
            var model = await _unitofWork.VehicleModel.GetById(id);
            return _mapper.Map<ResponseVehicleModel>(model);
        }

        public async Task<ResponseVehicleModel> UpdateStatusVehicleModel(Guid id, string status)
        {
            var model = await _unitofWork.VehicleModel.GetById(id);
        ...
        public async Task<ResponseVehicleModel> UpdateVehicleModel(Guid id, UpdateVehicleModel vehicleModel)
        {
            var model = await _unitofWork.VehicleModel.GetById(id);
